...
from slutprojekt_classes import *
import pickle
...
class Window:
    def __init__(self, root):
        ...
        # Bygger Top-frame
        self.btnSave = Button(self.topFrame, text="Spara", width=10, command=self.SaveToFile)
        ...
        self.root.bind("<Return>", lambda x: self.ShowMovielistBox)
    ...
    def ShowMovielistBox(self):
        show = Tk()
        Label(show, text="Nr").grid(row=0, column=0)
        Label(show, text="Title").grid(row=0, column=1)
        Label(show, text="Genre").grid(row=0, column=2)
        Label(show, text="Release").grid(row=0, column=3)
        Label(show, text="Seen").grid(row=0, column=4)

        for index, movie in enumerate (self.Movielist):
            Label(show, text=str(index + 1)).grid(row=index + 1, column=0)
            Label(show, text=movie.title).grid(row=index + 1, column=1, sticky = W)
            Label(show, text=movie.release).grid(row=index + 1, column=2, sticky = W)
            Label(show, text=movie.genre).grid(row=index + 1, column=3, sticky = W)
            if movie.seen == False:
                info = "Ej sett"
            ...
            Label(show, text=info).grid(row=index + 1, column=4, sticky = W)
